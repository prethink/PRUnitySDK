...
- для `Fade` окно берёт `CanvasGroup` контейнера или добавляет его. Во время закрытия
  `blocksRaycasts` выключен: нажатие по уходящему окну не проходит. Своё состояние группы
  (прозрачность, `blocksRaycasts`) окно может задать само: переход запоминает его перед
  каждым показом и закрытием, застав окно в покое, и возвращает к нему, а не к «видно
  и нажимается».
