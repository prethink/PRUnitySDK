...
**Отдавать `null` не стоит даже там, где «ничего не выбрано» — законное состояние.**
Объект, у которого поле не тронули, молча ничего не сделает, при том что в инспекторе
значение показано, — а искать такую поломку тяжело. Если состояние «ничего» нужно,
заводите его явным значением набора и ставьте первым: тогда оно и в списке видно,
и в коде сравнимо.
