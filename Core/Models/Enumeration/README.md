...
Ссылки можно держать списком или массивом — набор берётся из элемента, а не из коллекции:

```csharp
[SerializeField]
private List<EnumerationReference<EntityTypeEnumerations>> entityTypes = new();
```

Каждый пункт списка рисуется своим dropdown. Новый пункт добавляется пустым, а пустая
ссылка означает значение по умолчанию набора — выбирайте значение сразу после добавления.
...
абстрактное, как и `IncludeInherited`. Обычный ответ — первое объявленное значение,
для него есть `FirstOption`:
...
а инспектор показывает именно его. Это и есть смысл всей затеи: видимое в выпадашке
и выполняемое кодом должны совпадать.

**Не отдавайте `null` даже там, где «ничего не выбрано» — законное состояние.**
Объект с нетронутым полем ничего не сделает, хотя в инспекторе значение показано,
и такую поломку долго искать. Если состояние «ничего» нужно, заведите его явным
значением набора и поставьте первым: оно будет видно в списке и сравнимо в коде.

Порядок у `partial`-наборов зависит от того, в каком файле объявлено значение,
поэтому первое значение стоит закрепить атрибутом `EnumerationOrder`.
...
Окно `PRUnitySDK/Windows/Проверка проекта` находит все поля `EnumerationReference<T>`
в ассетах, префабах и на сценах и проверяет два случая:

- значение не выбрано — показывает, какое значение по умолчанию подставится, и может
  записать его в поле;
- сохранённая строка отсутствует в провайдере поля — так видно значения, которые удалили
  или переименовали.

Проверяются открытые сцены и все сцены проекта: закрытые окно открывает по очереди
и закрывает обратно. Правку объекта на сцене нужно сохранить самому — окно только
помечает сцену изменённой.
...
- Не переименовывайте сохранённые значения без миграции данных: сначала проверьте ссылки в окне `PRUnitySDK/Windows/Проверка проекта`.
