...
/// <para>
/// Глобальный масштаб времени физика учитывает сама: хост тикает её через
/// <c>Physics.Simulate(GameFixedDeltaTime)</c>, где шаг умножен на глобальный масштаб.
/// А вот слои <see cref="PRTimeScale"/> PhysX не поддерживает - симуляция одна на сцену.
/// База добавляет телу разницу между его слоем и глобальным темпом; откуда берётся слой,
/// решает наследник.
/// </para>
/// <para>
/// Замедление времени в k раз означает два разных множителя: скорость умножается на k,
/// а ускорение - на k², потому что путь равен v·(kt) + g·(kt)²/2. Поэтому мало умножить
/// velocity: без поправки к гравитации персонаж взлетал бы замедленно, а падал в обычном
/// темпе. Гравитация тела не отключается - прикладывается корректирующее ускорение
/// g·(k²-1), которое при k = 1 равно нулю.
/// </para>
/// </summary>
