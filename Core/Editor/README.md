...
`Summary` на вкладке `Overview` отображается компактной строкой `Label: Value`. Все показатели
помещаются в один ряд при обычной ширине; только в узком окне строка переносится на две или
четыре колонки. В развёрнутом окне основная область ограничена шириной `1000 px` и центрируется.
