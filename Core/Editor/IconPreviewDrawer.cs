...
public class IconPreviewDrawer : PropertyDrawer
{
    ...
    /// </summary>
    /// <remarks>
    /// Спрайт показывается сам собой, а у определения берётся иконка через
    /// <see cref="IIconProvider"/>, поэтому атрибут работает и с полем спрайта,
    /// и с полем валюты, предмета или награды.
    /// <para>
    /// Иконка бывает вычисляемой: награда отдаёт иконку выданного предмета.
    /// У недонастроенной награды такое свойство бросает исключение, а исключение
    /// из <c>OnGUI</c> ломает отрисовку всего окна.
    /// </para>
    /// </remarks>
