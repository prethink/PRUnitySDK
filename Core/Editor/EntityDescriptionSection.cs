...
/// </summary>
/// <remarks>
/// Описание хранится по-разному: в определении, в отдельном ассете или на компоненте.
/// Раздел показывает его в инспекторе сущности и подписывает источник, потому что правка
/// общего описания меняет его всем сущностям сразу.
/// <para>
/// Раздел не объявляет <c>[CustomEditor]</c>: два редактора на один тип Unity выбирает
/// произвольно, поэтому инспектор заводит проект, а раздел подмешивается в него. Создайте
/// раздел, вызывайте <see cref="Draw"/> из <c>OnInspectorGUI</c>, освобождайте
/// в <c>OnDisable</c>.
/// </para>
/// </remarks>
