...
/// </summary>
/// <remarks>
/// Сборщик билда вырезает код, до которого не дотягивается ни один явный вызов. Классы,
/// которые находит и создаёт SDK — фоновые задачи, источники резерва, обработчики
/// эффектов, — для него выглядят мёртвыми: ссылок на них в коде нет. В редакторе всё
/// работает, а в билде такой класс просто исчезает.
/// <para>
/// Помечать каждый класс <c>[Preserve]</c> вручную можно, но легко забыть, и ошибка
/// всплывёт только в собранной игре. Поэтому список собирается сам: по атрибутам
/// и интерфейсам, с которыми SDK работает через рефлексию. Наследовать свой атрибут
