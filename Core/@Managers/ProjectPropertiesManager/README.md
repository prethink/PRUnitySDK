...
У методов записи и удаления три флага:

| Параметр | Текущее поведение |
| --- | --- |
| `save` | при `true` вызывает `GameManager.Instance.SaveProjectData()` |
| `ignoreSaveCooldown` | при `true` запись идёт не дожидаясь кулдауна сохранения |
| `requiredNotify` | при `true` публикует событие изменения в `EventBus` (см. [Уведомления](#уведомления)) |

Порядок именно такой: два флага сохранения стоят рядом, потому что второй уточняет
первый и в отрыве от него смысла не имеет.
...
### Когда обходить кулдаун

Флаги отвечают на разные вопросы и не заменяют друг друга:

| Флаг | Значит |
| --- | --- |
| `save` | попытаться сохранить; внутри кулдауна вызов ничего не сделает, изменение уйдёт со следующей записью |
| `ignoreSaveCooldown` | писать прямо сейчас, кулдаун не ждать |

Обычной настройке хватает первого — она подождёт общего расписания.

Не подходит тому, за что игрок заплатил: покупке, отключению рекламы, награде. Между
списанием и следующим разрешённым сохранением игрок может закрыть вкладку, и покупка
пропадёт, а ресурсы останутся списанными — или наоборот.

```csharp
properties.SetBool("RemoveAds", true, ignoreSaveCooldown: true);
```

Флаг доходит до `GameManager.SaveProjectData(ignoreCooldown)`, который и решает,
можно ли писать прямо сейчас. Злоупотреблять им не стоит: кулдаун существует, чтобы
не дёргать диск и облако на каждое изменение.
