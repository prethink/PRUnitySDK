...
/// </summary>
/// <remarks>
/// Различает три вопроса. «Что за предмет» — <see cref="Category"/>, по ней собирают
/// открытые шапки отдельно от брейнротов. «Сколько штук есть» — <see cref="Count"/>,
/// меняется при выдаче и трате. «Открывал ли игрок этот предмет когда-либо» —
/// <see cref="IsOpened"/>, назад не отыгрывается: потратив последний ключ, игрок
/// не перестаёт знать о ключах, а купленный скин не должен снова оказаться в продаже.
/// <para>
/// Ссылки на само определение здесь нет. В сохранение всё равно попадал только
/// идентификатор, а при загрузке на его месте оказывалась пустая заглушка, у которой,
/// кроме идентификатора, ничего и не было. Строка честнее: видно, что именно хранится.
/// Определение по нему берут из каталога, когда оно действительно нужно.
/// </para>
/// </remarks>
