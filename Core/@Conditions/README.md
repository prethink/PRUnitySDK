...
| Тип | Назначение |
| --- | --- |
| `ICondition` | Контракт с методом `Evaluate(GameObject actor = null)` |
...
`Evaluate(actor)` получает объект, для которого проверяют правило. Хуки срабатывания
и урона передают инициатора или атакующего. Правила состояния мира по-прежнему вызывают
через `Evaluate()`; тогда `actor` равен `null`. Составные условия передают тот же объект
каждому вложенному правилу.
...
Правило для конкретного участника регистрируют функцией с `GameObject`:

```csharp
ConditionRegistry.Instance.Register(MyConditions.CanUseButton,
    actor => actor != null && actor.GetComponent<PlayerBase>() != null);
```
...
Для конкретного участника подпись выбирают через `ConditionDescriptions.Get(condition, actor)`.
