...
public partial class GameManager : MonoBehaviourSingletonBase<GameManager>
{
    ...
    ///// Ресурс контейнер.
    ///// </summary>
    //public ProjectResourceContainer ResourceContainer => resourceContainer;
...
private static extern void UnregisterPageVisibilityCallback();
    ...
    {
        gameDataStorage = PRUnitySDK.ResolveService<IGameDataStorage>();
        gameDataStorage.Load();

        LoadingData();
        AutoSaveHandler();

        EventBus.RaiseEvent<IReadyGameEvent>(invoke => invoke.OnReadyGame());
