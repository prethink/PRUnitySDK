...
### Завершение подавленного действия

Перегрузка `Publish(context, originalAction, suppressedAction)` вызывает `suppressedAction`, если pre-хук установил `Supercede`. Этот callback, как и оригинальное действие, завершается до post-хуков. Старые перегрузки сохраняют своё поведение.

HealthComponent использует эту перегрузку, чтобы post-хук видел заполненный `DamageHookEvent.Outcome` и при попадании, и при блоке/промахе. Уведомления локальных событий и EventBus приходят после post-этапа. Итог урона берите из `Outcome`: поздняя правка полей контекста не отменяет уже применённое изменение HP.