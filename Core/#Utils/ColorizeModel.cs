...
/// <remarks>
/// <para>
/// Нужен там, где модель временно показывают в другом виде: чёрный силуэт неизвестной
/// награды, подсветка выбранного, обозначение недоступного. Исходные цвета снимаются
/// при запуске, поэтому вернуть их можно в любой момент, не помня, какими они были.
/// </para>
/// <para>
/// Цвет задаётся через <c>MaterialPropertyBlock</c>: он меняет вид конкретного объекта,
/// не создавая ему личную копию материала. Копия жила бы до конца сцены и выбивала
/// объект из общей отрисовки, а перекрашивают обычно много объектов сразу.
/// </para>
/// <para>
/// Имя свойства настраивается: в URP цвет называется <c>_BaseColor</c>, во встроенном
/// конвейере — <c>_Color</c>.
/// </para>
/// </remarks>
