...
или поправить значение, нужно идти в Project, искать файл и возвращаться. Атрибут
раскрывает поля ассета прямо в компоненте:

```csharp
[field: SerializeField, InlineAsset] public PlayerStats Stats { get; protected set; }
```

Подпись поля работает треугольником: свернул — обычная ссылка, развернул — поля ассета
в рамке, редактируемые на месте. Правки идут в сам ассет и попадают в Undo.

Поля читаются у **присвоенного экземпляра**, а не у типа поля. Поэтому:
...
При множественном выделении блок полей не рисуется — у разных объектов ассеты разные,
и показывать значения первого было бы обманом. Вложенность ограничена тремя уровнями:
ассет со своим `[InlineAsset]` внутри раскрывается, но взаимные ссылки не уводят
инспектор в бесконечность.
