...
/// <remarks>
/// <para>
/// Активность лежит отдельным полем, потому что нужна почти всем и должна читаться
/// в отладчике без расшифровки. Остальное наследники <c>SaveableObjectState</c>
/// кладут своими ключами, не заводя новых полей в общих данных проекта.
/// </para>
/// <para>
/// Ключ — <see cref="EnumerationType{T}"/>: он несёт и имя значения, и его тип.
/// Поэтому <c>Set</c> и <c>TryGet</c> работают с настоящим типом, а не со строкой,
/// которую каждый наследник разбирал бы сам. Тот же приём, что у
/// <c>ProjectPropertiesManager</c> и <c>PRGameStorageService</c>.
/// </para>
/// <para>
/// Значения разложены по словарям на тип, как в <c>ProjectProperties</c>: так они
/// переживают запись в JSON без догадок о том, чем была строка <c>"1"</c> — числом,
/// признаком или текстом.
/// </para>
/// </remarks>
[Serializable]
