...
/// <remarks>
/// <para>
/// Нужен ради ключа: править его руками нельзя. Ключ связывает объект с записью
/// в сохранении, и опечатка в нём не ломает ничего заметного — просто состояние
/// перестаёт находиться, и объект молча встаёт в значения по умолчанию. Поэтому поле
/// показывается только для чтения, а новый ключ выдаётся кнопкой с подтверждением.
/// </para>
/// <para>
