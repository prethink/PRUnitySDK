...
/// <remarks>
/// <para>
/// Вешается на объект — сущность, обычный <c>MonoBehaviour</c> или пустышку рядом —
/// и отвечает на один вопрос: в каком виде объект должен появиться. Если о нём есть
/// запись в сохранении, применяется она; если записи нет, берутся значения по умолчанию
/// из инспектора. Так новый объект на уровне ведёт себя предсказуемо, а уже виденный
/// игроком возвращается таким, каким тот его оставил.
/// </para>
/// <para>
/// Из коробки хранится только активность. Всё остальное добавляют наследники через
/// <see cref="CaptureState"/> и <see cref="ApplyState"/>, складывая свои значения
/// в <see cref="SceneObjectState"/> по ключам <see cref="EnumerationType{T}"/> —
/// ключ несёт и имя значения, и его тип.
/// </para>
/// <para>
/// Важно, где стоит компонент. Выключенный объект не получает <c>Awake</c>, поэтому
/// сам себя обратно включить не сможет. Если объект может оказаться выключенным,
/// компонент вешают на живого соседа или родителя и указывают выключаемый объект
/// в поле цели.
/// </para>
/// </remarks>
public class SaveableObjectState : PRMonoBehaviour
{
    ...
    /// <remarks>
    /// <para>
    /// Отдельного вызова «вас скопировали» в Unity нет. Зато у каждого объекта сцены
    /// есть <c>GlobalObjectId</c>, и при копировании он не наследуется: у копии он свой.
    /// Записав рядом с ключом адрес объекта, которому ключ выдан, копию мы узнаём
    /// по расхождению — и выдаём ей собственный ключ.
    /// </para>
    /// <para>
    /// Без этого расставленная копированием россыпь — а её так и расставляют — ехала бы
    /// с одним ключом на всех: подобрал любой предмет, исчезли все.
    /// </para>
    /// <para>
    /// Тем же путём ключ меняется при переносе объекта в другую сцену: отличить перенос
    /// от копирования нечем. Сохранённое состояние такого объекта потеряется, и он
    ...
    /// </summary>
    /// <remarks>
    /// Два объекта с одним ключом пишут в одну запись и молча перетирают друг друга:
    /// последний сохранившийся побеждает, а игрок видит, что объект сам вернулся
    /// в прежнее состояние. Ловить это по факту тяжело, поэтому говорим сразу
    /// и называем обоих.
    /// </remarks>
