...
Данные проекта в `Awake` ещё не загружены, поэтому состояние применяется в два приёма.
...
не найдётся, объект встанет в значения по умолчанию, и ошибки в консоли при этом
не будет.
...
Руками ключ не правится, поле показано только для чтения. Опечатка в нём ничего
не ломает заметно: состояние перестаёт находиться, и объект встаёт в значения
по умолчанию. Такие ошибки ищут долго.
...
`Is Trigger` — без галки площадка становится препятствием, поэтому компонент
...
запись в JSON без догадок о том, чем была строка `"1"`. Незнакомый тип сразу бросает исключение.
