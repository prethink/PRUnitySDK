...
При `requiredSave: true` плавная операция запрашивает сохранение после завершения,
даже если последний кадр уже установил итоговый баланс. Передаётся и `ignoreSaveCooldown`.
Операция без изменения исходного значения сохранение не запускает.
