...
### Физика на паузе не останавливается

Своего монитора у `Rigidbody` больше нет — `RigidBodyPauseMonitor` удалён. Он сохранял
скорости и гасил gravity, но `isKinematic` не трогал из-за известной проблемы, то есть
полной остановки всё равно не давал, а стоял на объектах, где физических тел нет вовсе.

Логическая пауза `Time.timeScale` не меняет, поэтому PhysX продолжает считать: тело,
летящее в момент паузы, долетит. Тому, что на паузе двигаться не должно, нужен либо
собственный слой `PRTimeScale` с драйвером (`RigidbodyTimeScaleDriverBase`), либо
кинематическое тело и движение скриптом по `PRTime.GameDeltaTime`.
