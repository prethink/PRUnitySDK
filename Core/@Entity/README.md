...
- **`EntityInfoProvider` не работает.** Поле `EntityInfoData` не помечено `[SerializeField]`
  и нигде не присваивается, поэтому свойство `EntityInfo` всегда возвращает `null`.
  Механизм переопределения описания через отдельный компонент фактически отключён —
  `EntityUtils.GetEntityInfo` всегда получает `overrideEntityInfo == null`.
- **`ContainerEntityBase` обходит паузу.** Класс объявляет собственный Unity-метод
  `OnTriggerEnter` вместо `PROnTriggerEnter`, поэтому подбор срабатывает и во время
  логической паузы. Рядом в том же классе `PROnCollisionEnter` использован правильно —
  расхождение стоит устранить.
