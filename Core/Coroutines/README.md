...
## Когда корутина не подходит

Для периодической работы, у которой нет владельца на сцене — автосохранение, офлайн-доход,
опрос серверного времени — используйте [BackgroundTasks](../BackgroundTasks/README.md).
Бесконечная корутина в таком случае требует объекта-хозяина, теряется при смене сцены
и не даёт ни счётчиков запусков, ни защиты от череды ошибок; задачи же обслуживаются
одним общим тиком, поэтому их количество не увеличивает число корутин.
