...
`Add` и `Buy` запускают сохранение, но между записями действует кулдаун: вызов внутри
него ничего не сделает, и изменение уйдёт со следующим сохранением. Для набегающей
мелочи это правильно — иначе каждая монета дёргала бы диск.

Разовой покупке этого мало. Между списанием и следующим разрешённым сохранением игрок
может закрыть вкладку: тогда предмет останется, а валюта нет — или наоборот.

```csharp
wallet.Buy(currency, price, ignoreSaveCooldown: true);
```

Решение принимает вызывающий: только он знает, разовая это покупка или ставка в цикле.
Флаг проходит через `ResourceManager` до `GameManager.SaveProjectData(ignoreCooldown)`.
