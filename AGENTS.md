...
- `Id` — произвольная строка. У предметов каталогов это GUID, у ресурса — ключ `Enumeration` (`Coin`, `Yan`), у проектного типа что угодно. Сравнивайте идентификаторы как строки; `Guid.TryParse` в проверке молча отсечёт всё, кроме одного вида предметов.
