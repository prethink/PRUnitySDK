...
**Фреймворк для игр на Unity.** Не библиотека, которую вызывают по мере надобности:
он задаёт форму приложения — точку входа, порядок инициализации, жизненный цикл
компонентов, модель данных и способ, которым игра расширяет его поведение.

Разница практическая. Библиотеку вы вызываете; фреймворк вызывает вас:

- игра запускается из bootstrap-сцены, а порядок инициализации задают
  `MethodHookAttribute` и этапы, а не ваш `Start`;
- компоненты наследуют `PRMonoBehaviour` и получают `PRUpdate`, паузу, игровое время
  и готовность через `ReadySignal` — обычный `MonoBehaviour` мимо этого проходит;
- поведение расширяется не правкой ядра, а подключением к нему: хуки урона, провайдеры
  с автопоиском по атрибутам, фабрики окон, трекеры сущностей;
- данные игры описаны его моделью — каталоги, определения предметов, награды, проекты.

Название осталось историческим: строка `PRUnitySDK` зашита в пути `Resources`, пункты
меню и сериализованные ассеты, поэтому переименование — отдельная задача с миграцией.

> [!WARNING]
> Проект находится в активной разработке. Публичные API, структура каталогов и процесс
> инициализации могут изменяться. Перед обновлением фиксируйте используемый commit.
...
Во фреймворке шесть способов вклиниться в чужое поведение. Они не взаимозаменяемы: каждый решает
