...
**Фреймворк для игр на Unity.** Он задаёт форму приложения: точку входа, порядок
инициализации, жизненный цикл компонентов, модель данных и способ, которым игра
...
- компоненты наследуют `PRMonoBehaviour` и получают `PRUpdate`, паузу, игровое время
  и готовность через `ReadySignal`; обычный `MonoBehaviour` всего этого не получает;
- поведение расширяется подключением к ядру, а не правкой ядра: хуки урона, провайдеры
...
- [Conditions](Core/@Conditions/README.md) — переиспользуемые правила «когда это разрешено»; рядом с действиями, но отдельно от них: действие совершает поступок, условие только спрашивает. Оттуда же берут подпись для игрока: чего не хватает, чтобы условие выполнилось
