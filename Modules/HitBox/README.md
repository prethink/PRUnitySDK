...
| `HitBoxExtensions` | Получение хитбокса и здоровья из коллайдера, столкновения или объекта |
...
```csharp
if (other.TryGetHitBox(out EntityHitBoxBase hitBox))
    hitBox.TakeDamage(attacker, weapon, damage, other);
else if (other.TryGetHealth(out HealthComponent health))
    health.TakeDamage(attacker, weapon, damage);
```

Порядок именно такой: **бить лучше в хитбокс**. Он умножает урон на зону и помечает
крит, а напрямую в здоровье эти правила проходят мимо — выстрел в голову засчитается
как обычный.
...
Все три есть для `Collider`, `Collision` и `GameObject`.
