...
public class LocalizationWindow : ExtendedEditorWindow
{
    ...
    private void DrawTable(SerializedProperty listProp)
    {
        ...
        for (int i = 0; i < listProp.arraySize; i++)
        {
            ...
            var langProp = element.FindPropertyRelative(nameof(LocalizationRow.LangData).GetBackingField());
        ...
        SerializedProperty langProp)
    {
        ...
        if (GUILayout.Button("⎘", GUILayout.Width(30)))
        {
            ...
            // LANG DATA
            var srcLang = src.FindPropertyRelative(nameof(LocalizationRow.LangData).GetBackingField());
            var dstLang = dst.FindPropertyRelative(nameof(LocalizationRow.LangData).GetBackingField());
    ...
    private void DrawAdd(SerializedProperty listProp)
    {
        ...
        if (GUILayout.Button("+ Add Key", GUILayout.Height(25)))
        {
            ...
            var langProp = newElement.FindPropertyRelative(nameof(LocalizationRow.LangData).GetBackingField());
            EnsureLangSize(langProp);
        }
    }
