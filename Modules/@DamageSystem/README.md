...
Добавьте `DamageSound` на объект с `HealthComponent` или на его дочерний объект и заполните
список `Damage Clips`. `Playback Order` выбирает последовательное воспроизведение по кругу
или случайный клип при каждом ударе; пустые элементы списка пропускаются. Если здоровье
находится в другом месте, укажите его в поле `Health Component`.
`Min Pitch` и `Max Pitch` задают случайную высоту звука для каждого удара; одинаковые
значения дают фиксированный pitch.
Компонент подписывается на локальные события здоровья при включении и снимает подписки
при выключении. Звук играет один раз при применённом уроне больше нуля, включая
смертельный удар и удар по бессмертной цели; промах, блок и лечение его не запускают.
