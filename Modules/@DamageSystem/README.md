...
| `DamageRules`, `DamageRule`, `MultiplyDamageRule` | Правила урона игры: проект, сцена, код — через хук урона |
...
Хуки обычно вешают на цель, и слушатель проверяет, что удар по нему. **Правила урона**
(папка `Rules`) описывают правила игры целиком — «боты не ломают блоки», «игроки
не бьют друг друга» — без компонентов на каждой цели и без правок в оружии. Правило
видит любой удар: касание, ближний бой, снаряд, взрыв.

### Сервис `DamageRules`

Один на всю игру. Правила приходят из трёх мест и дальше неотличимы:

| Откуда | Как | Сколько действует |
| --- | --- | --- |
| проект | `PRUnitySDK.Settings.DamageRules → Rules` | на всех сценах; ставятся на старте SDK |
| сцена | компонент `DamageRuleComponent` | пока компонент включён |
| код | `DamageRules.Instance.Add(rule, owner)` / `Remove(owner)` | пока не снимут |

К удару правила приходят **через существующий хук**: каждое регистрируется
в `HookManager` отдельным слушателем со своим `Order`. `TakeDamage` публикует хук
на каждом ударе, так что сервис спрашивается всегда, а правила стоят в общем порядке
с условиями, неуязвимостью и сопротивлениями. Второго конвейера рядом с хуками нет.

Владелец (`owner`) нужен, чтобы снять ровно свои правила: режим снимает свои, сцена —
свои, чужие остаются.

```csharp
// на время боя с боссом: игроки не бьют друг друга
var rule = new MultiplyDamageRule(0f) { Attackers = PlayerTypeFlags.Human };
rule.AddVictimType(EntityTypeEnumerations.Player);
...
`DamageRule` — обычный объект (`[SerializeReference]`), поэтому одно и то же правило
кладут в настройки, на сцену или создают в коде. Фильтры общие:

| Фильтр | Что задаёт | Пусто |
| --- | --- | --- |
| `Attackers` (`PlayerTypeFlags`) | чьи удары: игроков, ботов, NPC | любой атакующий, в том числе не игрок |
| `Victim Types` (виды сущностей) | по кому | любая цель |

Готовое правило — `MultiplyDamageRule`:

| `Action` | Что происходит |
| --- | --- |
| `Multiply` | урон умножается; `0` — удар без урона: попадание засчитано, эффекты играют, цель цела |
| `Block` | удар отклонён (`Blocked`), эффекты урона не играют |

`Order` следует таблице выше: `Multiply` — на `0`, `Block` принято ставить на `−200`
(`MultiplyDamageRule.CreateBlock()` так и делает). Порядок читается, когда правило
попадает в сервис: поменять его на ходу — снять правило и добавить снова.
...
`OnHit` — попадание, которое стоит показать игроку: эффекты, звук, цифра урона. Приходит
сразу после локального `OnDamageProcessed`, только для засчитанного удара и только если
урон больше нуля. Удар без урона (правило урона умножило его на 0) приходит, если проект
включил `PRUnitySDK.Settings.Health.ZeroDamageIsHit`. Промах, блок и смерть через `Kill()`
не приходят никогда. То же решение даёт `HealthComponent.IsHit(outcome)` — им пользуются те,
кому попадание приходит не из события.

На `OnHit` подписаны `HealthSound` и эффекты урона. Так одна настройка решает для всей
игры, виден ли удар без урона, а подписчики сами ноль не проверяют.
...
5. Вызывается локальный `OnDamageProcessed`.
   Если исход — попадание (`IsHit`), следом вызывается локальный `OnHit`.
...
Добавьте `HealthSound` на объект с `HealthComponent` или на его дочерний объект и заполните
списки `Damage Clips` и `Death Clips`. `Playback Order` выбирает последовательное
воспроизведение по кругу или случайный клип; пустые элементы списка пропускаются, у каждого
списка своя очередь. Если здоровье находится в другом месте, укажите его в поле
`Health Component`. `Min Pitch` и `Max Pitch` задают случайную высоту звука; одинаковые
значения дают фиксированный pitch. Громкость и pitch общие для обоих списков.

Компонент подписывается на локальные события здоровья при включении и снимает подписки
при выключении. На одно событие играет один звук:

- попадание (`OnHit`) играет звук урона: урон больше нуля, включая удар по бессмертной
  цели, а при `Health.ZeroDamageIsHit` — и удар без урона;
- смерть играет звук смерти, в том числе смерть через `Kill()`;
- если `Death Clips` пуст, смертельный удар звучит как обычный урон, а `Kill()` проходит
  без звука, потому что урона в нём нет;
- промах, блок и лечение звука не дают.

Для позиции используется точка попадания, затем место жертвы из `DamageOutcome`.
